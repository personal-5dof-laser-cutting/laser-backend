...
def test_travel_cost(cuts: list[TrapezoidalCut]):
    ...
    assert geo.calculate_travel_cost(material_thickness, as_cycle=False) == 0

    geo.cuts[1] = geo.cuts[1].flipped_direction()

    assert geo.calculate_travel_cost(material_thickness, as_cycle=False) == geo.cuts[
        0
    ].travel_time_to(geo.cuts[1], material_thickness) + geo.cuts[1].travel_time_to(
        geo.cuts[2], material_thickness
    )
